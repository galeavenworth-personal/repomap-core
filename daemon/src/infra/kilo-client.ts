/**
 * Kilo Client — HTTP helpers for kilo serve interaction.
 *
 * Extracted from factory-dispatch.ts (ovm.7) to isolate the kilo serve
 * HTTP surface: session creation, prompt dispatch, message fetching,
 * idle detection, child monitoring, and result extraction.
 *
 * This module is a shared dependency for both factory-dispatch (CLI path)
 * and Temporal activities (durable path). The Temporal path has its own
 * richer monitoring via pollUntilDone; these helpers are the legacy
 * polling path used by runDispatch and will be deprecated when
 * runDispatch migrates to the Temporal client (see ovm-6 spec).
 *
...
/** Monitor result from session polling. */
...
// ── Logger type (shared with factory-dispatch) ───────────────────────────
 ...
 *
 * @deprecated Will be replaced by Temporal pollUntilDone. See ovm-6 spec.
 */
...
/** Minimal config subset needed by monitorSession. */
