...
from conftest import build_minimal_artifacts, write_jsonl
from contract.artifacts import DEPS_SUMMARY_JSON, SYMBOLS_JSONL
...
def _build_extended_artifacts(artifacts_dir: Path) -> None:
    """Build artifacts with cycles, violations, complexity, and security."""
    build_minimal_artifacts(
        artifacts_dir,
        extra_integration_records=[
            {
    ...
    write_jsonl(
        artifacts_dir / _COMPLEXITY_JSONL,
        [
            {
...
def artifacts_dir(tmp_path: Path) -> Path:
    ...
    _build_extended_artifacts(path)
    return path
...
class TestQueryEngine:
    ...
    def test_dot_notation_filter_matches_nested_field(self, tmp_path: Path) -> None:
        ...
        _build_extended_artifacts(artifacts_dir)
